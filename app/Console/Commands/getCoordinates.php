...
class getCoordinates extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($data as $key => $value) {
            ...
            // $url = "https://maps.googleapis.com/maps/api/place/findplacefromtext/json?fields=business_status,formatted_address,rating,geometry,icon,icon_mask_base_uri,icon_background_color,name,photo,place_id,plus_code,type&input={$location_name}&inputtype=textquery&key=AIzaSyBDpoXg_iNv-dAAXEM1mek_sSKqLijSNOI";
            ...
            foreach($geos["candidates"] as $k => $val){
                if(!Str::contains($val["formatted_address"], 'Kalimantan Barat')) {
                    $value->location = null;
                    $value->save();
                    continue;
                }
